...
for fgender in ["F","M"] :
    for date in ["2020-03-13"] :
        for ftriptype in ["microtrip","bus","private_transport"] :       
            request = "/generatematrix?format=json&project={}&date={}&ftriptype={}&fgender={}".format(projectID, date, ftriptype, fgender)
...
final_data =  pd.pivot_table(data, index=['ProjectID', 'Date', 'Origin', 'Destination', 'Modo'], columns='Gender', values='Trips')
final_data.to_csv("matriz1b_genero_2020-03-13.csv")
